...
@section ('content')
    ...
    @forelse ($users as $user)
        @if ($admins < 5)
        <div class="border border-light p-2 ml-2 text-decoration-none"><div class="text-uppercase">{{$user->pseudo}}</div>
            ...
            @endif</div></a>
        @else
            <div class="border border-light p-2 ml-2 text-decoration-none"><div>{{$user->pseudo}}</div>
            @if($user->role->slug === 'ADMI')
                    <a href="{{ route('users.edit',$user->id) }}"><div><button type="button" class="btn btn-sm btn-danger">Destituer</button></div>
            @endif
            </div>
